...
def  findnearst(position, path):
    ...
    if minindex<50:
        ...
        if distance<1:
            alfa=np.matrix([[4*distance**2, 0, 0],[0, 4*distance**2, 0],[0, 0, 1]])
        else:
            alfa=np.matrix([[4, 0, 0],[0, 4, 0],[0, 0, 1]])    # 0<alfa<1  self defination
        beta=np.matrix([[0.5, 0, 0],[0, 0.5, 0],[0, 0, 0]])     # 0<beta<1  self defination
        ...
        if minindex<(len(x)-51):
            ...
            if minindex<(len(x)-1):
                ...
                alfa=np.matrix([[0.3, 0, 0],[0, 0.3, 0],[0, 0, 1]])    # 0<alfa<1  self defination
