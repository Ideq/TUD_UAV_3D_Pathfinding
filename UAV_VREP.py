...
def followPath2(clientID,path,goal):
    ...
    while (distest > 0.05):
        ...
        vec,vec_path,pnear=pathfollowing.findnearst(pos,path)
        #vec=vec1[0]        
        #print vec
        ...
        if abs(vec[0,2])>1:
            ...
            xvelo=xvelo*((np.pi-abs(dangle))/np.pi)**45*(1-abs(vec[0,2]))**20
            yvelo=yvelo*((np.pi-abs(dangle))/np.pi)**10*(1-abs(vec[0,2]))**5
